...
class TestNoBackslashEscapesBypassRegression:
    ...
    def test_backslash_quote_does_not_escape_in_single_quoted(self):
        # Any backslash in the query is now rejected outright — closes
        # the parser-mismatch bypass regardless of which interpretation
        # the server uses.
        q = "SELECT 'abc" + chr(92) + "'; DROP TABLE users; --'"
        _bad(q, match="Backslashes are not permitted")
...
class TestBackslashRejection:
    """Top-level regression: any backslash in the query is rejected
    outright, regardless of context (string, comment, identifier).

    This is the defense-in-depth fix for the entire parser-mismatch class:
    a Python-side parser cannot match BOTH MySQL modes (NO_BACKSLASH_ESCAPES
    on and off), so rejecting backslashes eliminates the mismatch surface.
    """

    def test_backslash_in_string_literal_rejected(self):
        q = "SELECT 'a" + chr(92) + "b'"
        _bad(q, match="Backslashes are not permitted")

    def test_backslash_in_double_quoted_rejected(self):
        q = 'SELECT "a' + chr(92) + 'b"'
        _bad(q, match="Backslashes are not permitted")

    def test_backslash_outside_string_rejected(self):
        # Even outside any string, backslash is rejected (no MySQL syntax uses it)
        q = "SELECT 1 " + chr(92) + " FROM users"
        _bad(q, match="Backslashes are not permitted")

    def test_backslash_in_like_pattern_rejected(self):
        # Documented trade-off: LIKE patterns with \% must be rewritten
        q = "SELECT 1 FROM users WHERE name LIKE 'foo" + chr(92) + "%'"
        _bad(q, match="Backslashes are not permitted")

    def test_backslash_in_comment_rejected(self):
        q = "SELECT 1 /* contains " + chr(92) + " backslash */"
        _bad(q, match="Backslashes are not permitted")
